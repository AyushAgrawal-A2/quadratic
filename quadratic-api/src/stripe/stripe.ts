import { BillingPlanType, SubscriptionStatus, type Team } from '@prisma/client';
import { UserTeamRoleSchema } from 'quadratic-shared/typesAndSchemas';
import Stripe from 'stripe';
import { trackEvent } from '../analytics/mixpanel';
import { determinePlanTypeFromSubscription } from '../billing/planHelpers';
...
export const upgradeSubscriptionPlan = async (
  ...
  const previousMetadataBillingPlanType = subscription.metadata?.plan_type;
  ...
  const newBillingPlanType = planType === 'business' ? BillingPlanType.BUSINESS : BillingPlanType.PRO;
  try {
    await dbClient.team.update({
      where: { id: team.id },
      data: {
        planType: newBillingPlanType,
      },
    });
  } catch (dbError) {
    ...
    try {
      await stripe.subscriptions.update(team.stripeSubscriptionId, {
        ...
        metadata: {
          plan_type: previousMetadataBillingPlanType ?? '',
        },
      });
    } catch (revertError) {
      logger.error('Failed to revert Stripe subscription after DB failure — manual intervention required', {
        ...
        teamUuid: team.uuid,
        subscriptionId: team.stripeSubscriptionId,
        attemptedBillingPlanType: planType,
        previousPriceId,
        revertError,
      });
    }
...
export const createCheckoutSession = async (
  ...
  let finalBillingPlanType = planType;
  if (!finalBillingPlanType) {
    const proPriceId = await getProPriceId();
    finalBillingPlanType = priceId === proPriceId ? 'pro' : 'business';
  }
  ...
  return stripe.checkout.sessions.create({
    ...
    subscription_data: {
      metadata: {
        plan_type: finalBillingPlanType === 'business' ? 'BUSINESS' : 'PRO',
      },
    },
  });
};
...
export const updateTeamStatus = async (
  ...
  // Get subscription to extract plan type from metadata or price lookup key
  const subscription = await stripe.subscriptions.retrieve(stripeSubscriptionId);
  let planTypeFromMetadata = subscription.metadata?.plan_type as BillingPlanType | undefined;

  // Fallback: infer plan type from price lookup key for existing subscriptions without metadata
  if (!planTypeFromMetadata && subscription.items?.data?.length > 0) {
    const seatItem = subscription.items.data.find((item) => item.price.recurring?.usage_type !== 'metered');
    const lookupKey = seatItem?.price?.lookup_key;
    if (lookupKey === STRIPE_LOOKUP_KEY_BUSINESS) {
      planTypeFromMetadata = BillingPlanType.BUSINESS;
    } else if (lookupKey === STRIPE_LOOKUP_KEY_PRO) {
      planTypeFromMetadata = BillingPlanType.PRO;
    }
  }

  const planType = determinePlanTypeFromSubscription(stripeSubscriptionStatus, planTypeFromMetadata);
