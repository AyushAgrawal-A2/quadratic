...
export function getGenAIApiArgs(
  ...
  const messages: GenAIContent[] = promptMessages.reduce<GenAIContent[]>((acc, message) => {
    if (isInternalMessage(message)) {
      ...
      const genaiMessage: GenAIContent = {
        role: message.role,
        parts: validToolResults.map((toolResult) => ({
          functionResponse: {
            name: toolResult.id,
...
export function convertParametersToGenAISchema(parameter: AIToolArgsPrimitive | AIToolArgsArray | AIToolArgs): Schema {
  // Handle array of types (union types)
  if (Array.isArray(parameter.type)) {
    // Handle nullable array type: ['array', 'null']
    const types = parameter.type as string[];
    if (types.length === 2 && types.includes('array') && types.includes('null')) {
      return {
        type: Type.ARRAY,
        items: convertParametersToGenAISchema((parameter as AIToolArgsArray).items),
        nullable: true,
      };
    }
