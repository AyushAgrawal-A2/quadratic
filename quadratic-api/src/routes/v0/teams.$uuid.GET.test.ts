...
describe('GET /v0/teams/:uuid', () => {
  describe('get a team you belong to', () => {
    ...
    it('returns cancelAtPeriodEnd in billing response', async () => {
      const team = await dbClient.team.findUniqueOrThrow({
        where: { uuid: '00000000-0000-4000-8000-000000000001' },
      });

      // Set team with an active subscription that is set to cancel at period end
      await dbClient.team.update({
        where: { id: team.id },
        data: {
          stripeCustomerId: 'cus_test123',
          ...
          stripeCurrentPeriodEnd: new Date(Date.now() + 86400 * 30 * 1000),
        },
      });
      ...
      await request(app)
        ...
        .expect(200)
        .expect((res) => {
          expect(res.body.billing.cancelAtPeriodEnd).toBe(true);
          expect(res.body.billing.currentPeriodEnd).toBeDefined();
        });
    });

    it('returns cancelAtPeriodEnd=false when subscription is not cancelled', async () => {
      const team = await dbClient.team.findUniqueOrThrow({
        where: { uuid: '00000000-0000-4000-8000-000000000001' },
      });

      // Set team with an active subscription that is NOT set to cancel
      await dbClient.team.update({
        where: { id: team.id },
        data: {
          stripeCustomerId: 'cus_test123',
      ...
      await request(app)
        ...
        .expect(200)
        .expect((res) => {
          expect(res.body.billing.cancelAtPeriodEnd).toBe(false);
        });
    });

    it('triggers lazy sync when stripeCancelAtPeriodEnd is null and team has subscription', async () => {
      const team = await dbClient.team.findUniqueOrThrow({
        where: { uuid: '00000000-0000-4000-8000-000000000001' },
      });

      // Set team with a subscription but stripeCancelAtPeriodEnd is null (existing user scenario)
      await dbClient.team.update({
        where: { id: team.id },
        data: {
          stripeCustomerId: 'cus_test123',
          ...
          stripeSubscriptionStatus: 'ACTIVE',
          stripeCancelAtPeriodEnd: null, // Existing user, not synced yet
        },
      });

      // Mock Stripe response with cancel_at_period_end = true
      mockCustomersRetrieve.mockResolvedValue({
        id: 'cus_test123',
        deleted: false,
        subscriptions: {
      ...
      await request(app)
        ...
        .expect(200)
        .expect((res) => {
          // Should have synced and returned the cancel status
          expect(res.body.billing.cancelAtPeriodEnd).toBe(true);
        });

      // Should have triggered the lazy sync
      expect(mockCustomersRetrieve).toHaveBeenCalledTimes(1);
      ...
      expect(updatedTeam.stripeCancelAtPeriodEnd).toBe(true);
    });

    it('does not trigger lazy sync when stripeCancelAtPeriodEnd is already set', async () => {
      const team = await dbClient.team.findUniqueOrThrow({
        where: { uuid: '00000000-0000-4000-8000-000000000001' },
      });

      // Set team with subscription and stripeCancelAtPeriodEnd already set
      await dbClient.team.update({
        where: { id: team.id },
        data: {
          stripeCustomerId: 'cus_test123',
          stripeSubscriptionId: 'sub_existing',
          stripeSubscriptionStatus: 'ACTIVE',
          stripeCancelAtPeriodEnd: false, // Already synced
        },
      });
      ...
      // Should NOT have triggered the lazy sync since stripeCancelAtPeriodEnd is already set
      expect(mockCustomersRetrieve).not.toHaveBeenCalled();
    });

    it('does not trigger lazy sync when team has no subscription', async () => {
      // Team has no subscription by default
      await request(app)
        .get(`/v0/teams/00000000-0000-4000-8000-000000000001`)
        .set('Authorization', `Bearer ValidToken team_1_owner`)
        .expect(200)
        .expect((res) => {
          // cancelAtPeriodEnd should be undefined for teams without subscription
          expect(res.body.billing.cancelAtPeriodEnd).toBeUndefined();
        });

      // Should NOT have triggered the sync since there's no subscription
      expect(mockCustomersRetrieve).not.toHaveBeenCalled();
    });
  });
});
