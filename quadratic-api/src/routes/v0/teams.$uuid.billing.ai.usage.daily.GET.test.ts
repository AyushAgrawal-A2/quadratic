    ...
    it('returns daily costs per user', async () => {
      ...
      await request(app)
        ...
        .expect(({ body }) => {
          ...
          const yesterdayEntry = body.dailyCosts.find((d: { date: string }) => d.date === toDateString(yesterday));
          const todayEntry = body.dailyCosts.find((d: { date: string }) => d.date === toDateString(today));
    ...
    it('handles mixed overageEnabled records across days', async () => {
      ...
      await request(app)
        ...
        .expect(({ body }) => {
          ...
          const day1Str = toDateString(day1);
          const day2Str = toDateString(day2);
