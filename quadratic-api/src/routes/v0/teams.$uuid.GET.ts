...
async function handler(req: Request, res: Response<ApiTypes['/v0/teams/:uuid.GET.response'] | ResponseError>) {
  ...
  // Do this if: 1) explicitly requested, 2) status indicates subscription may be stale,
  // or 3) existing user with subscription but no cancel status synced yet (lazy sync)
  const needsLazySync = team.stripeCancelAtPeriodEnd === null && team.stripeSubscriptionId !== null;
  ...
  if (shouldUpdateBilling) {
    try {
      ...
      logger.error('Failed to sync billing during team load', {
        teamUuid: uuid,
        teamId: team.id,
  ...
  const { editableFileIds, isOverLimit, totalFiles, maxEditableFiles } = await getFileLimitInfo(
    dbTeam,
    isPaidPlan,
    userMakingRequestId
  );
  ...
  const response = {
    ...
    billing: {
      ...
      cancelAtPeriodEnd: dbTeam.stripeCancelAtPeriodEnd ?? undefined,
      planType,
      usage,
    },
    userMakingRequest: {
    ...
    fileLimit: {
      isOverLimit,
      totalFiles,
      maxEditableFiles: maxEditableFiles === Infinity ? undefined : maxEditableFiles,
    },
  };
