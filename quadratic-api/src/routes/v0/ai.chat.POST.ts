...
import { canMakeAiRequest, getPlanType, isFreePlan } from '../../billing/planHelpers';
...
async function handler(req: RequestWithUser, res: Response<ApiTypes['/v0/ai/chat.POST.response']>) {
  ...
  if (messageType === 'userPrompt') {
    ...
    if (exceededBillingLimit) {
      const planType = getPlanType(ownerTeam);
      const allowOveragePayments = ownerTeam.allowOveragePayments;
