...
async function handler(req: RequestWithUser, res: Response<ApiTypes['/v0/teams/:uuid/billing/ai/usage.GET.response']>) {
  ...
  // canMakeAiRequest fetches the user's cost internally, so reuse it to avoid a redundant query
  const [{ allowed, userCost: currentMonthAiCost }, userBudgetLimit] = await Promise.all([
    canMakeAiRequest(team, userId),
