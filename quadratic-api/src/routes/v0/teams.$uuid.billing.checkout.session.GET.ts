  ...
  if (isOnPaidPlan) {
    ...
    if (plan === 'business' && currentPlanType === 'PRO') {
      ...
      try {
        ...
        logger.info('Downgrading subscription from Business to Pro', {
          teamUuid: uuid,
          teamId: team.id,
        });

        await upgradeSubscriptionPlan(team, proPriceId, 'pro');

        const successUrl = new URL(redirectSuccess);
        successUrl.searchParams.set('subscription', 'downgraded');
        const data: ApiTypes['/v0/teams/:uuid/billing/checkout/session.GET.response'] = {
          url: successUrl.toString(),
        };
        return res.status(200).json(data);
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        logger.error('Error downgrading subscription plan', {
          error: errorMessage,
          teamUuid: uuid,
          plan,
        });
        return res.status(500).json({
          error: {
            message: 'Failed to downgrade subscription plan. Please try again later.',
          },
        });
      }
