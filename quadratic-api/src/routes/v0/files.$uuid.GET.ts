...
async function handler(req: RequestWithOptionalUser, res: Response<ApiTypes['/v0/files/:uuid.GET.response']>) {
  ...
  if (req.query.updateBilling === 'true') {
    try {
      ...
      logger.error('Failed to sync billing during file load', {
        fileUuid: req.params.uuid,
        teamId: ownerTeam.id,
        stripeCustomerId: ownerTeam.stripeCustomerId,
        error,
      });
    }
    ...
    if (updatedTeam) {
      ...
      ownerTeam.stripeCurrentPeriodStart = updatedTeam.stripeCurrentPeriodStart;
  ...
  const isEditRestricted = await requiresUpgradeToEdit(ownerTeam, id, userId);
