...
const aiSourceToAIChatSource: Record<AISource, AIChatSource> = { ...AIChatSource };
 ...
 * Errors are logged but don't throw. Overage reporting to Stripe
 * is fire-and-forget so it never delays the caller.
...
export async function trackAICost({
  ...
  try {
    ...
    reportAndTrackOverage(teamId).catch((error) => {
      logger.error('Background overage reporting failed', { error, teamId });
    });
  } catch (error) {
