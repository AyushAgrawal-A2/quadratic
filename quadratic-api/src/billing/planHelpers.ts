...
/**
 * Determine the billing plan type from a subscription status and optional metadata hint.
 * - ACTIVE + BUSINESS metadata → BUSINESS
 * - ACTIVE → PRO (default for active subscriptions)
 * - anything else → FREE
 */
export const determinePlanTypeFromSubscription = (
  subscriptionStatus: SubscriptionStatus | null,
  metadataPlanType?: BillingPlanType | null
): BillingPlanType => {
  if (subscriptionStatus === SubscriptionStatus.ACTIVE) {
    return metadataPlanType === BillingPlanType.BUSINESS ? BillingPlanType.BUSINESS : BillingPlanType.PRO;
  }
 ...
 * Otherwise, infer from subscription status via determinePlanTypeFromSubscription.
 */
export const getPlanType = (team: Team | DecryptedTeam): BillingPlanType => {
  if (team.planType) {
    return team.planType;
  }
  return determinePlanTypeFromSubscription(team.stripeSubscriptionStatus);
};
...
export const isBusinessPlan = (team: Team | DecryptedTeam): boolean => {
  return getPlanType(team) === BillingPlanType.BUSINESS;
};
...
export const isProPlan = (team: Team | DecryptedTeam): boolean => {
  return getPlanType(team) === BillingPlanType.PRO;
};
...
export const isFreePlan = (team: Team | DecryptedTeam): boolean => {
  return getPlanType(team) === BillingPlanType.FREE;
};
 ...
 */
export const getDefaultAllowanceForPlan = (planType: BillingPlanType): number => {
  switch (planType) {
    case BillingPlanType.FREE:
      return 0;
    case BillingPlanType.PRO:
      return AI_ALLOWANCE_PRO;
    case BillingPlanType.BUSINESS:
      return AI_ALLOWANCE_BUSINESS;
    default: {
      const _exhaustiveCheck: never = planType;
      throw new Error(`Unhandled plan type: ${_exhaustiveCheck}`);
    }
  }
};
...
export const hasExceededAllowance = async (team: Team | DecryptedTeam, userId: number): Promise<boolean> => {
  ...
  if (planType === BillingPlanType.FREE) {
    return false;
  }
...
export const canMakeAiRequest = async (
  ...
  userId: number
): Promise<{ allowed: boolean; reason?: string; userCost: number }> => {
  const planType = getPlanType(team);

  if (planType === BillingPlanType.FREE) {
    return { allowed: true, userCost: 0 };
  }
  ...
  if (planType === BillingPlanType.PRO) {
    if (allowancePerUser === 0) {
      return { allowed: true, userCost: 0 };
    }
    ...
    if (roundToCents(userCost) >= roundToCents(allowancePerUser)) {
      return { allowed: false, reason: 'Monthly AI allowance exceeded', userCost };
    }
    return { allowed: true, userCost };
  }

  // Business plan: fetch billing data in parallel, skipping queries that aren't needed.
  // Team breakdown + user count are only needed when a team budget limit is configured.
  // User budget limit is only needed when overage payments are enabled (without overage,
  // users are either within allowance or blocked entirely, so the budget cap is moot).
  const needsTeamBudgetData = team.teamMonthlyBudgetLimit != null;
  const needsUserBudgetData = !!team.allowOveragePayments;
  ...
  const [userBreakdown, budgetLimit, teamBreakdown, userCount] = await Promise.all([
    ...
    needsUserBudgetData ? getUserBudgetLimit(team.id, userId) : null,
    needsTeamBudgetData ? getBillingPeriodAiCostBreakdownForTeam(team.id, start, end) : null,
    needsTeamBudgetData ? dbClient.userTeamRole.count({ where: { teamId: team.id } }) : 0,
  ]);
  ...
  let exceededTeamBudget = false;
  if (teamBreakdown && team.teamMonthlyBudgetLimit != null) {
    ...
    exceededTeamBudget = roundToCents(teamBilledOverage) >= roundToCents(team.teamMonthlyBudgetLimit);
  }

  if (exceededUserAllowance) {
    if (team.allowOveragePayments) {
      if (exceededUserBudget) {
        return { allowed: false, reason: 'User monthly budget limit exceeded', userCost };
      }
      if (exceededTeamBudget) {
        return { allowed: false, reason: 'Team monthly budget limit exceeded', userCost };
      }
      return { allowed: true, userCost };
    }
    return { allowed: false, reason: 'Monthly AI allowance exceeded', userCost };
