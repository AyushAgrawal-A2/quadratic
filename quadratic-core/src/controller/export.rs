        ...
        if !cell_value.is_html() && !cell_value.is_image() {
            match &cell_value {
                ...
                CellValue::RichText(spans) => {
                    let formats = build_rich_text_formats(spans);
                    let segments: Vec<(&Format, &str)> = formats
                        .iter()
                        .zip(spans.iter())
                        .map(|(fmt, span)| (fmt, span.text.as_str()))
                        .collect();
                    worksheet.write_rich_string(row, col, &segments)
                }
...
/// Builds `rust_xlsxwriter::Format` objects for each TextSpan in a RichText cell.
fn build_rich_text_formats(spans: &[TextSpan]) -> Vec<Format> {
    spans
        .iter()
        .map(|span| {
            ...
            if span.strike_through == Some(true) {
                fmt = fmt.set_font_strikethrough();
            }
            if let Some(ref color) = span.text_color {
                fmt = fmt.set_font_color(color.as_str());
            }
            if let Some(size) = span.font_size {
                let excel_size = (size + FONT_SIZE_DISPLAY_ADJUSTMENT) as f64;
                fmt = fmt.set_font_size(excel_size);
            }
            fmt
        })
        .collect()
