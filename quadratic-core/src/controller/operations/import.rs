        ...
        for sheet_name in sheets.iter() {
            ...
            for (y, row) in range.rows().enumerate() {
                for (x, cell) in row.iter().enumerate() {
                    let cell_value = match cell {
                        ...
                        ExcelData::RichText(value) => convert_excel_rich_text(value),
                    };
            ...
            for (y, x, style) in style_range.cells() {
                ...
                let is_rich_text = matches!(
                    sheet.columns.get_value(&pos),
                    Some(CellValue::RichText(_))
                );

                // font formatting (only if font info exists and cell is not rich text,
                // since rich text spans carry their own per-run formatting)
                if !is_rich_text
                    && let Some(font) = style.get_font()
                {
                    font.is_bold()
                    ...
                    {
                        let internal_size =
                            size.round() as i16 - FONT_SIZE_DISPLAY_ADJUSTMENT;
...
/// Converts calamine RichText to Quadratic CellValue::RichText
fn convert_excel_rich_text(rich_text: &calamine::RichText) -> CellValue {
    let spans: Vec<TextSpan> = rich_text
        .iter()
        .map(|run| {
            ...
            if let Some(font) = &run.font {
                ...
                if font.has_strikethrough() {
                    span.strike_through = Some(true);
                }
                if let Some(color) = &font.color
                    && !color.is_black()
                {
                    span.text_color = Some(color.to_string());
                }
                if let Some(size) = font.size
                    && size != DEFAULT_FONT_SIZE
                {
                    span.font_size =
                        Some(size.round() as i16 - FONT_SIZE_DISPLAY_ADJUSTMENT);
                }
            }
    ...
    if spans.iter().all(|s| s.is_plain()) {
        CellValue::Text(spans.iter().map(|s| s.text.as_str()).collect())
    } else {
...
mod test {
    ...
    #[test]
    fn test_convert_excel_rich_text_with_formatting() {
        use calamine::{Font, FontWeight, FontStyle, RichText, TextRun, UnderlineStyle};

        let mut rt = RichText::new();
        rt.push(TextRun::with_font(
            "bold".to_string(),
            Font::new().with_weight(FontWeight::Bold),
        ));
        rt.push(TextRun::with_font(
            " italic".to_string(),
            Font::new().with_style(FontStyle::Italic),
        ));
        rt.push(TextRun::with_font(
            " underline".to_string(),
        ...
        let result = convert_excel_rich_text(&rt);
        match result {
            CellValue::RichText(spans) => {
                ...
                assert_eq!(spans[1].italic, Some(true));
                assert_eq!(spans[1].bold, None);

                assert_eq!(spans[2].text, " underline");
                assert_eq!(spans[2].underline, Some(true));

                assert_eq!(spans[3].text, " strike");
                assert_eq!(spans[3].strike_through, Some(true));
            }
            _ => panic!("Expected RichText, got {:?}", result),
        }
    }

    #[test]
    fn test_convert_excel_rich_text_with_color_and_size() {
        use calamine::{Font, RichText, TextRun};
        ...
        rt.push(TextRun::new("plain".to_string()));

        let result = convert_excel_rich_text(&rt);
        match result {
            CellValue::RichText(spans) => {
                assert_eq!(spans.len(), 2);
                assert_eq!(spans[0].text, "red big");
                assert_eq!(spans[0].text_color, Some("#FF0000".to_string()));
                // Excel 14pt → internal 18 (14 - (-4) = 18)
                assert_eq!(spans[0].font_size, Some(18));

                assert_eq!(spans[1].text, "plain");
                assert!(spans[1].is_plain());
            }
            _ => panic!("Expected RichText, got {:?}", result),
        }
    }
    ...
    #[test]
    fn test_convert_excel_rich_text_plain_fallback() {
        use calamine::{RichText, TextRun};

        let mut rt = RichText::new();
        rt.push(TextRun::new("hello ".to_string()));
        rt.push(TextRun::new("world".to_string()));
    ...
    #[test]
    fn test_convert_excel_rich_text_skips_default_color_and_size() {
        use calamine::{Font, FontWeight, RichText, TextRun};

        let mut rt = RichText::new();
        rt.push(TextRun::with_font(
            "bold with defaults".to_string(),
            Font::new()
        ...
        match result {
            CellValue::RichText(spans) => {
                ...
                assert_eq!(spans[0].bold, Some(true));
                assert_eq!(spans[0].text_color, None);
                assert_eq!(spans[0].font_size, None);
            }
            _ => panic!("Expected RichText, got {:?}", result),
        }
    }
