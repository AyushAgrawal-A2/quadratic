...
`;
  ...
  async getSummaryContext(): Promise<ChatMessage[]> {
    ...
    const sheetNames = joinListWith({ arr: sheets.sheets.map((sheet) => `'${sheet.name}'`), conjunction: 'and' });
      ...
      return [
        {
          ...
          content: [
            createTextContent(
              `# File Summary\n\n## Sheets\n\nFile has ${sheetCount} ${pluralize('sheet', sheetCount)}, named ${sheetNames}.\n\nAll sheets are currently empty with no data.`
            ),
          ],
        ...
        },
        {
          ...
          content: [
            createTextContent(
              `I see the file has ${sheetCount} empty ${pluralize('sheet', sheetCount)}. How can I help you?`
            ),
          ],
          contextType: 'fileSummary',
        },
