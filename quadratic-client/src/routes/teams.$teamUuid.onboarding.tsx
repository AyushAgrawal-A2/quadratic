...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  // If the user wants to upgrade to Pro or Business, redirect to the billing subscribe route.
  // The create file route passes the subscription param to the file route, which handles
  // subscription success (updating billing state).
  const selectedPlan = result.data?.['team-plan'];
  if (selectedPlan === 'pro' || selectedPlan === 'business') {
    return redirectDocument(ROUTES.TEAM_BILLING_SUBSCRIBE(teamUuid, { returnTo: newFilePath, plan: selectedPlan }));
  }
